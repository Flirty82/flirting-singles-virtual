// backend/middleware/diamondMember.js
...
const verifyDiamondMemberSocket = async (socket, next) => {
  try {
    ...
    if (userData.subscriptionId) {
      ...
      // Allow socket connection if verified within last 3 days
      if (daysSinceVerification > 3) {
        try {
          ...
          if (subscription.status !== 'ACTIVE' || 
              subscription.plan_id !== 'P-339627571K154671RNAQCLQA') {
            return next(new Error('Subscription inactive'));
          }
          ...
          console.error('Socket PayPal verification error:', paypalError);
          return next(new Error('Subscription verification failed'));
        }
      }
    }
    ...
    console.error('Socket diamond verification error:', error);
    next(new Error('Authentication failed'));
  }
};

module.exports = {
  diamondMemberMiddleware,
  verifyDiamondMemberSocket
};